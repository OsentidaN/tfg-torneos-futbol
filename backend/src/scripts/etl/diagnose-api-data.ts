...
async function diagnoseApiData() {
    ...
    console.log('Este proceso tomará varios minutos...\n');
    ...
    try {
        ...
        async function checkSeason(leagueId: number, leagueName: string, year: number) {
            ...
            try {
                ...
                console.log(`${result.teams > 0 ? 'OK' : 'X'} ${result.teams} equipos`);
                ...
                if (result.sampleFixtureId) {
                    ...
                    try {
                        ...
                        console.log(`${result.hasFixtureStats ? 'OK' : 'X'} ${result.hasFixtureStats ? 'Disponibles' : 'No disponibles'}`);
                    } catch (err) {
                    ...
                    try {
                        ...
                        console.log(`${result.hasFixtureEvents ? 'OK' : 'X'} ${result.hasFixtureEvents ? 'Disponibles' : 'No disponibles'}`);
                    } catch (err) {
                    ...
                    try {
                        ...
                        console.log(`${result.hasFixtureLineups ? 'OK' : 'X'} ${result.hasFixtureLineups ? 'Disponibles' : 'No disponibles'}`);
                    } catch (err) {
                    ...
                    try {
                        ...
                        console.log(`${result.hasFixturePlayerStats ? 'OK' : 'X'} ${result.hasFixturePlayerStats ? 'Disponibles' : 'No disponibles'}`);
                    } catch (err) {
        ...
        for (const [league, data] of Object.entries(results)) {
            ...
            for (const r of data) {
                const check = (val: boolean | number) =>
                    typeof val === 'boolean' ? (val ? 'OK' : 'X') : val.toString().padStart(3);
        ...
        console.log('\nINTERPRETACIÓN:');
        console.log('   OK = Datos disponibles');
        console.log('   X = Datos NO disponibles');
        console.log('   Número = Cantidad de registros encontrados\n');

        console.log('RECOMENDACIÓN:');
        console.log('   • Importa SOLO los años con OK en los endpoints que necesites');
